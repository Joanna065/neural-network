...
def kernel_size_same_pad_experiment(model_dict, train_dict):
    def exp_generator():
        for kernel_size, padding in [(3, 1), (5, 2), (7, 3), (11, 5)]:
            model_dict['kernel_size'] = kernel_size
...
if __name__ == "__main__":
    ...
    dir_name = 'kernel_size_same_pad'
    results = run_experiment(kernel_size_same_pad_experiment(model_dict_conv, train_dict),
                             out_dir=dir_name, test_data=test_data)
    plot_val_loss(results, dirname=dir_name)
    plot_val_accuracy(results, dirname=dir_name)
    plot_time_bar(results, dirname=dir_name, time_unit='min.')
